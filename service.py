...
class Iris(object):
    ...
    def __init__(self):
        self.gbm = rt.InferenceSession("model.onnx")
        self.input_name = self.gbm.get_inputs()[0].name
        self.label_name = self.gbm.get_outputs()[0].name

    def on_get(self, req, resp, index):
        if index < X_count:
            y_pred = self.gbm.run([self.label_name], {self.input_name: X_scaled[index].reshape(1, -1).astype(
                np.float32)})[0]
